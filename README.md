...
## Create new user role and add permission to user role(CustomRole):
Apigee edge comes with built in roles and with several permission level.However, you can create new role that includes the xact permission you want.
 
    Refer to [How to create custom roles through management api]:(https://docs.apigee.com/api-platform/system-administration/managing-roles-api),
    (https://docs.apigee.com/api-platform/system-administration/permissions),(https://apidocs.apigee.com/management/apis/post/organizations/%7Borg_name%7D/userroles),()


    For example, the customroles goal is used below to create new user role with different permission level[Post,Get,Delete]:
      
        mvn apigee-config:customroles -Ptest -Dapigee.config.options=create
